...
public class Globals {
	...
	/*
	 * @Author - shardha Remove al the cookies
	 */
	 ...
	 * //Data base
	 * 
	 * public void ERtakeScreenshot (WebDriver driver,String OEM, String
	 * reportName){ DateFormat df = new SimpleDateFormat("yyyyMMdd"); String
	 * data = df.format(new Dalistte()); log("Date = "+data);
	 * 
	 * String myString = data+"_" +OEM +"_" +reportName; log(myString); myString
	 * = myString.replaceAll("/", "_").toLowerCase(); log(myString); myString =
	 * myString.replaceAll(" ", "_").toLowerCase(); log(myString);
	 * 
	 * File scrFile =
	 * ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE); // now save
	 * the screenshot to a file some place try { FileUtils.copyFile(scrFile, new
	 * File(".\\screens\\screenshot_"+myString+".png"));
	 * log("Screenshot Saved"); } catch (IOException e) { e.printStackTrace(); }
	 * }
	 */
	 ...
	 */
	public void ERtakeScreenshotError(WebDriver driver, String OEM,
			String reportName) {
		...
		try {
			FileUtils.copyFile(scrFile,
					new File(".\\screens\\Errors\\Error_screenshot_" + myString
							+ ".png"));
	...
	public void openurl() {
		System.setProperty("webdriver.chrome.driver",
				"D://lib//ChromeDriver_Folder//chromedriver_win32//chromedriver.exe");
	...
	public String captureScreen() throws IOException {
		...
		File source = ((TakesScreenshot) augmentedDriver)
				.getScreenshotAs(OutputType.FILE);
		path = "D:\\workspace\\AmazonTest\\ScreenCapture" + source.getName();
	...
	static {
		...
		System.setProperty("webdriver.chrome.driver", directoryPath
				+ "D:\\lib\\chromedriver_win32\\chromedriver.exe");
	...
	public void autoit() throws IOException {
		...
		Runtime.getRuntime().exec(
				"/FutureGroup/lib/DriverRelated/AutoIt/fg.exe");
	...
	public void mouseHoverOn(WebElement ele) {
	...
	public void selectFromListBox(List<WebElement> elementList, String selection) {
		try {
			...
			log("Error : ["
					+ selection
					+ "] is not present in the list box or list box contains no value : "
	...
	public static boolean VerifyTextPresent(final String text, int timeout)
			throws HeadlessException, IOException, AWTException {
		log("Wait for Text : " + text + " to be present on UI for maximum ["
				+ timeout + "] seconds");
		try {
			...
			String ErrorMsg = "Failed wait for Text Present [" + text
					+ "] request within timeout of : " + timeout
	 ...
	 * @param int timeout
	 * @throws HeadlessException
	 *             , IOException, AWTException VerifyTextNotPresent. To Verify
	 *             if a text is not present on the page Also wait for text not
	 *             present. It can also verify fade away messages.
	 * 
	 */
	...
	public static boolean VerifyTextNotPresent(final String text, int timeout)
			throws HeadlessException, IOException, AWTException {
		log("Wait for Text : " + text
				+ " to be not present on UI for maximum [" + timeout
				+ "] seconds");
		try {
			new WebDriverWait(driver, timeout) {
			}.until(new ExpectedCondition<Boolean>() {
			...
			String ErrorMsg = "Failed wait for Text not to be Present [" + text
					+ "] request within timeout of : " + timeout
					+ " seconds  at url:" + driver.getCurrentUrl();
			log(ErrorMsg);
			StackTraceElement ste = new Exception().getStackTrace()[1];
		...
		log("Verify Text [" + text + "] "
				+ "Is not Present on the UI : Passed ");
		assertTrue(true);
		return true;
	}
	...
	public static String switchToLatestTabOrWindow() {
		...
		driver.switchTo().window(
				getWindowHandles().toArray()[getWindowHandles().size() - 1]
						.toString());
	...
	public static String switchToNextTabOrWindow() {
		...
		driver.switchTo().window(
				getWindowHandles().toArray()[Math.max(
						getWindowHandles().size() - 1, pos + 1)].toString());
		return driver.getTitle();
	}
	...
	public static String switchToPreviousTabOrWindow() {
		...
		driver.switchTo().window(
				getWindowHandles().toArray()[Math.min(0, pos - 1)].toString());
		return driver.getTitle();
	}

	// Wait until an object is visible before interacting with it
	public static void WebDriverExplicitWait(WebDriver driver,
			int timeoutInSeconds, String by, String locator) {
		WebDriverWait wait = new WebDriverWait(driver, timeoutInSeconds);
	...
	public boolean switchWindowByTitle(String title) {
		...
		List<String> availableWindows = new ArrayList<String>(
	...
	private static String getFrameworkBrowserNameFromCapabilitiesName(
	...
	public static void browserSelectGrid() {
		...
		log("Request received for browser : " + browserName
				+ " having version as : " + browserVersion
		...
		if (browserName.equalsIgnoreCase(FW_IE_NAME)) {
			...
			if (browserAutoSave) {
				...
				fp.setPreference("browser.download.manager.showWhenStarting",
						false);
		...
		try {
			...
			log("There is an Error in getting Remote webdriver instance : "
					+ e.getMessage());
		}
		...
		driverInfo = "Initializing remote web driver: "
				+ remoteServerUrl.toString() + " -> " + sessionID
				+ ". Selenium Server version: " + "2.53" + ". ";
		driverInfo += "OS: " + remoteCapabilities.getPlatform() + ". ";
		driverInfo += "Browser: " + browserName + " Requested version: "
				+ browserVersion + ". Obtained version: "
		...
		if (checkObtainedBrowser) {
			...
			String obtainedBrowserName = getFrameworkBrowserNameFromCapabilitiesName(remoteCapabilities
					.getBrowserName());
			...
			if (!browserVersion.equalsIgnoreCase(obtainedBrowserVersion)
				...
				throwException = true;
				exceptionMessage = "browserName or browserVersion are not the requested ones. Requested: "
						+ browserName
						+ " "
						+ browserVersion
						+ ". Obtained: "
						+ obtainedBrowserName + " " + obtainedBrowserVersion;
			}
